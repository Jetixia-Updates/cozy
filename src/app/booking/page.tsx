        ...
        <AnimatePresence mode="wait">
          ...
          {step === 'rooms' && (
            ...
            >
              {rooms.map((room) => (
                ...
                >
                  <div className="bg-gradient-to-br from-primary-500 to-primary-700 p-4 sm:p-6 text-white">
                    <h3 className="text-xl sm:text-2xl font-bold mb-1 sm:mb-2">{room.name}</h3>
                    <p className="text-primary-100 text-sm sm:text-base">{room.type}</p>
                  </div>
                  ...
                  <div className="p-4 sm:p-6">
                    <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                      <div className="flex items-center space-x-reverse space-x-2 text-gray-600 dark:text-gray-400">
                        ...
                        <span className="text-sm sm:text-base">{room.capacity} {i18n.language === 'ar' ? 'شخص' : 'people'}</span>
                      </div>
                      <div className="flex items-center space-x-reverse space-x-2 text-gray-600 dark:text-gray-400">
                        <MapPin className="w-4 h-4 sm:w-5 sm:h-5" />
                        <span className="text-sm sm:text-base">{i18n.language === 'ar' ? 'الطابق' : 'Floor'} {room.floor}</span>
                      </div>
                    </div>
                    
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                      {room.amenities.map((amenity) => (
                        <div key={amenity} className="flex items-center space-x-reverse space-x-1 bg-gray-100 dark:bg-gray-700 px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm">
            ...
            >
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-2 sm:space-y-0">
                  <div>
                    ...
                    <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">{selectedRoom.type}</p>
                  </div>
                  <button
                    onClick={() => {
                      setStep('rooms')
                    ...
                    className="px-3 sm:px-4 py-2 text-sm sm:text-base text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition self-start sm:self-auto"
                  >
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-3 sm:p-6">
                ...
                <div className="mb-6 sm:mb-8">
                  <div className="bg-gradient-to-b from-gray-300 to-gray-200 dark:from-gray-600 dark:to-gray-700 rounded-t-2xl sm:rounded-t-3xl py-2 sm:py-3 text-center text-xs sm:text-sm text-gray-600 dark:text-gray-300 font-medium">
                    {i18n.language === 'ar' ? 'الشاشة / المنصة' : 'Screen / Stage'}
                  </div>
                </div>

                {/* Legend */}
                <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 mb-4 sm:mb-6 text-xs sm:text-sm">
                  <div className="flex items-center space-x-reverse space-x-1.5 sm:space-x-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-green-500 rounded-md sm:rounded-lg"></div>
                    <span className="text-gray-600 dark:text-gray-400">{i18n.language === 'ar' ? 'متاح' : 'Available'}</span>
                  </div>
                  <div className="flex items-center space-x-reverse space-x-1.5 sm:space-x-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-primary-500 rounded-md sm:rounded-lg"></div>
                    <span className="text-gray-600 dark:text-gray-400">{i18n.language === 'ar' ? 'مختار' : 'Selected'}</span>
                  </div>
                  <div className="flex items-center space-x-reverse space-x-1.5 sm:space-x-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-red-500 rounded-md sm:rounded-lg"></div>
                    <span className="text-gray-600 dark:text-gray-400">{i18n.language === 'ar' ? 'محجوز' : 'Booked'}</span>
                  </div>
                </div>

                {/* Seats Grid */}
                <div className="flex flex-col items-center space-y-2 sm:space-y-3 overflow-x-auto pb-2">
                  {Array.from(new Set(seats.map(s => s.row))).map((row) => (
                    <div key={row} className="flex items-center space-x-reverse space-x-1.5 sm:space-x-2">
                      <span className="w-6 sm:w-8 text-center font-bold text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                        {row}
                      </span>
                      {seats
                        .filter(s => s.row === row)
                        .map((seat) => (
                          <motion.button
                            ...
                            className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-md sm:rounded-lg font-medium text-xs sm:text-sm transition-all group ${
                          ...
                          >
                            ...
                            {seat.status === 'booked' && seat.bookedUntil && (
                              <div className="absolute hidden group-hover:block bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-gray-900 text-white text-[10px] sm:text-xs rounded-lg whitespace-nowrap z-10">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white rounded-xl hover:from-primary-700 hover:to-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition font-medium shadow-lg text-sm sm:text-base"
                  >
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 md:p-8">
                ...
                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
                  <button
                    ...
                    className="w-full sm:flex-1 px-4 sm:px-6 py-3 sm:py-4 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition font-medium text-sm sm:text-base"
                  >
                    {i18n.language === 'ar' ? 'رجوع' : 'Back'}
                  </button>
                  <button
                    onClick={handleConfirmBooking}
                    className="w-full sm:flex-1 px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white rounded-xl hover:from-primary-700 hover:to-primary-600 transition font-medium shadow-lg text-sm sm:text-base"
                  >
          ...
          {step === 'confirmation' && selectedRoom && (
            <motion.div
              ...
              className="max-w-2xl mx-auto"
            >
              <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 md:p-8 text-center">
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                ...
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4 px-2">
                ...
                <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-3 sm:mb-4 px-2">
                  ...
                  {isLoggedIn && (
                    <span className="block mt-2 font-medium text-primary-600 dark:text-primary-400 break-words">
                      {userEmail}
                    </span>
                  )}
                </p>
                ...
                {isLoggedIn && (
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-primary-50 dark:bg-primary-900/20 rounded-xl">
                    <p className="text-xs sm:text-sm text-primary-800 dark:text-primary-200 flex items-center justify-center flex-wrap gap-1">
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg sm:rounded-xl p-4 sm:p-6 text-right space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">{i18n.language === 'ar' ? 'رقم الحجز:' : 'Booking Number:'}</span>
                    <span className="font-bold text-xs sm:text-sm text-gray-900 dark:text-white break-all text-left">#{Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
                  </div>
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">{i18n.language === 'ar' ? 'الغرفة:' : 'Room:'}</span>
                    <span className="font-bold text-xs sm:text-sm text-gray-900 dark:text-white text-left">{selectedRoom.name}</span>
                  </div>
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">{i18n.language === 'ar' ? 'المقاعد:' : 'Seats:'}</span>
                    <span className="font-bold text-xs sm:text-sm text-gray-900 dark:text-white text-left break-words">{selectedSeats.join(', ')}</span>
                  </div>
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">{i18n.language === 'ar' ? 'التاريخ:' : 'Date:'}</span>
                    <span className="font-bold text-xs sm:text-sm text-gray-900 dark:text-white text-left">{selectedDate}</span>
                  </div>
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">{i18n.language === 'ar' ? 'الوقت:' : 'Time:'}</span>
                    <span className="font-bold text-xs sm:text-sm text-gray-900 dark:text-white text-left">
                      {selectedStartTime} - {calculateEndTime(selectedStartTime, selectedDuration)}
                    </span>
                  </div>
                  <div className="flex justify-between items-start gap-2 pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-600">
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">{i18n.language === 'ar' ? 'المبلغ الإجمالي:' : 'Total Amount:'}</span>
                    <span className="text-lg sm:text-2xl font-bold text-primary-600 dark:text-primary-400 text-left">
                ...
                <Link
                  href="/"
                  className="inline-block w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white rounded-xl hover:from-primary-700 hover:to-primary-600 transition font-medium shadow-lg text-sm sm:text-base"
                >
